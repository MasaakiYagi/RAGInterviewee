...
async def main():
    ...
    if st.button("Start Interaction"):
        try:
            recorded_data = await ai_assistant.record_audio()
            transcript_text = await ai_assistant.transcribe_audio(recorded_data)
            st.session_state.messages.append(("user", transcript_text))
            if transcript_text:
                assistant_content = await ai_assistant.run_thread_actions(transcript_text)
                await ai_assistant.text_to_speech(assistant_content)
                st.session_state.messages.append(("assistant", assistant_content))
